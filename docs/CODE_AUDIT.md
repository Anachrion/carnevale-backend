...
### B-P3-1 · Inconsistent error-response shapes — FIXED (2026-07-06)
...
**Resolution:** extracted a `RendersApiErrors` concern with one `render_error` that accepts a message (filed under `base`), an `ActiveModel::Errors` (`to_hash`), or a ready-made hash, always emitting `{ errors: { <attribute> => [messages] } }`. Every controller now routes through it (BaseController + games/lists/list_entries + the Devise-derived registrations/passwords, which don't share a base class). Both former shapes were already structurally the same `{errors: {key: [strings]}}` and match the documented `ValidationErrors` OpenAPI schema, so output is byte-identical and the request specs (which assert exact JSON) still pass — the win is one guaranteed code path that can't drift.

### B-P3-2 · Inconsistent success shapes / status codes — FIXED (documented as intentional, 2026-07-06)
...
**Resolution:** the variance is intentional and client-coupled, not accidental: `update_counters`/`update_stats` return the mutated **entry state** because the Flutter client applies it as an optimistic update on the tapped model (`_applyEntryState`), `draw_agendas` returns just the drawn **agenda delta**, and in all three the full game already reaches both players via `broadcast_state!` — returning the whole game on these hot paths would regress the F-P1-5/B-P2-4 work and, being a contract change, would force an OpenAPI-client regeneration and Flutter changes. `create`→`201` vs `update`→`200` is correct REST, not an inconsistency. Homogenizing would be a net regression, so instead the deliberate deviations are now documented in-code at each site (and here) so they read as intentional; response bodies and status codes are unchanged.

### B-P3-3 · Dead and broken jbuilder views — FIXED (2026-07-06)
...
**Resolution:** all three deleted (the `app/views/api/v1/lists/` directory is now empty and gone); every list response goes through `ListSerializer`.

### B-P3-4 · Routes generate nonexistent actions — FIXED (2026-07-06)
...
**Resolution:** `resources :lists, only: %i[index show create update destroy]`.

### B-P3-5 · Dead code — FIXED (2026-07-06): `Spell.choosable` scope (`spell.rb:8`); `ListValidationService` `adding:` path (`list_validation_service.rb:2-8,34-39`, only specs use it); empty `catalog.rb` module; `list_json` `with_entries: false` default branch (`base_controller.rb:12,22`, never passed).
**Resolution:** removed the `choosable` scope and the empty `app/models/catalog.rb` (Zeitwerk defines the `Catalog` namespace implicitly). Dropped the `adding:` parameter from `ListValidationService` and simplified `projected_items`; the three spec-only `adding:` examples were converted to persisted-entry equivalents (one kept as a multi-entry summation test, two removed as duplicates of existing persisted-entry cases). The `list_json with_entries: false` branch was already gone with the B-P2-8 serializer refactor.

### B-P3-6 · Duplication — FIXED (2026-07-06): `user_json` copy-pasted in `sessions_controller.rb:20-22`, `registrations_controller.rb:32-34`, `passwords_controller.rb:29-31` (move to `User`); `total_cost` in `base_controller.rb:18` vs `list.rb:22`; `refresh_list_selection_validity` in `gang/entry.rb:25-27` vs `gang/entry_spell.rb:14-16` (extract concern); `DRAW_ORIGINS`/`DISCARD_ORIGINS` (`games_controller.rb:4-5`) duplicate & can drift from `AgendaEvent::ORIGINS_BY_ACTION` (`agenda_event.rb:4-8`).
**Resolution:** the three `user_json` copies now share a `UserSerializer` (chosen over a `User` method to fit the B-P2-8 serializer layer; output identical). `refresh_list_selection_validity` + its `after_commit` moved into a `RefreshesListSelectionValidity` concern; `Gang::Entry`/`Gang::EntrySpell` include it and just supply `#selection_validity_list`. `DRAW_ORIGINS`/`DISCARD_ORIGINS` now derive from `Encounter::AgendaEvent::ORIGINS_BY_ACTION` (draw excludes the server-internal `initial`/`recycle`). The `total_cost` duplication was already resolved by B-P2-2.

### B-P3-7 · Consistency nits — FIXED (2026-07-06): string-enum fields split between `enum` (`game.rb:10`, `player.rb:13`, `spell.rb:5`) and `validates inclusion` (`player.rb:16`, `agenda_event.rb:14`); two different temp-position hacks in the ordering services (`list_sorting_service.rb:15-16` negative positions vs `list_entry_reorder_service.rb:16` `position: 0`, the latter bypassing the `greater_than: 0` validation via `update_columns`); magic `75` threshold in `list_validation_service.rb:75`; `AgendaEvent` unique index `(game_player_id, agenda_id, action)` has no model `validates uniqueness` → raw `RecordNotUnique` 500.
**Resolution:** magic `75` → `ListValidationService::LEADER_REQUIRED_ABOVE_POINTS`. Added `AgendaEvent`'s `validates :agenda_id, uniqueness: { scope: %i[game_player_id action] }` so a duplicate is a clean validation error rather than a raw `RecordNotUnique` (the double-draw path is also still guarded upstream). Unified the reorder service's temp slot to a **negative** sentinel matching `ListSortingService`'s convention (out of the valid 1..N range). The enum-vs-inclusion split was left as-is **by design** and documented in both models: `role`/`action` keep `validates inclusion` because a bad value must surface as a validation error (a spec, `game_player_spec.rb:13`, builds `role: "referee"` and expects invalidity — an `enum` would raise `ArgumentError` instead), whereas the defaulted state-machine columns (`status`/`visibility`/`discipline`) use `enum`.
...
### F-P3-5 · Config & correctness nits — FIXED (2026-07-06)
...
**Resolution:** each nit fixed. The host was already made env-driven during the F-P1-1 cable work (`String.fromEnvironment('API_HOST', …)` + `API_USE_TLS`/`API_KEY` defines), so only noted. `ProfileX.cardReferenceId` now returns `int?` (null when the profile has no printed card) and `_add` guards on null, so a cardless profile is never hired with a `0` id. `isTokenExpired` now returns `true` for a missing/unreadable `exp` (re-auth rather than trust a token of unknown validity) — the auth-service spec was updated to assert the corrected behaviour. `stopWatching` calls `notifyListeners()` after clearing `currentGame`. `card_viewer` uses `math.pi` / `math.pi / 2`. `flutter analyze` clean; 20 tests pass.

### F-P3-6 · Performance nits — FIXED (2026-07-06)
...
**Resolution:** `gang_builder` caches the filtered+sorted hire list in `_visibleProfiles`, recomputed only when its inputs change (search text, sort field/dir, profiles load) instead of re-sorting on every rebuild; `_buildTabContent` now builds only the active tab (`_tab == list ? … : …`) rather than an `IndexedStack` that laid out both eagerly. `cards` debounces keystroke search (300ms Timer, cancelled on dispose) so it fires at most one network call per pause, and caches the sorted view in `_sorted`, recomputed only on results/sort changes (faction-toggle and load still search immediately). `flutter analyze` clean; 20 tests pass.
