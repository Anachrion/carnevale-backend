...
### F-P2-3 · Massive UI-chrome duplication across screens — PARTIALLY FIXED (2026-07-05: AppBackground done)
**Resolution (background scaffold):** extracted `AppBackground` (`lib/widgets/app_background.dart`) — the bg image + optional blurred scrim + optional SafeArea — and replaced the ~25-line copy-pasted scaffold in all 10 screens (`home`, `cards`, `account`, `settings`, `gangs`, `game_home`, `game_session`, `gang_viewer`, `gang_builder`, `reset_password`). `flutter analyze` clean (0 errors). Remaining sub-items below (GlassPanel reuse, bottom-sheet chrome, shared header/state/decoration widgets) are still open.
